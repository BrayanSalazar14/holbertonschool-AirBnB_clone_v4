document.addEventListener('DOMContentLoaded', (event) => {
    ...
    boton.addEventListener('click', () => {
        ...
        const textHeading = document.querySelector('.amenities h4').textContent;
        const amenities = textHeading.split(',');
        ...
        .then(data => {
            data.forEach(place => {
                fetch(`http://127.0.0.1:5001/api/v1/places/${place.id}/amenities`)
                .then(response => response.json())
                .then(result => {
                    result.forEach(data => {
                        amenities.forEach(amenity => {
                            if (data.name === amenity) {
